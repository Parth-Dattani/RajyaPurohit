...
class RegistrationStepperScreen extends GetView<RegistrationController> {
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                _buildNavigationButtons(), // ➔ Step 0 પર આ ઓટોમેટિક છુપાઈ જશે
              ],
            ),
          ),
        ),
  ...
  Widget _buildMobileStep(BuildContext context) {
    return Column(
      children: [
        ...
        // ➔ ⚡ વેબ/લેપટોપ માટે બે ભાગ (Tabs) સિલેક્ટર
        // ➔ ⚡ મોબાઈલ અને વેબ બંને માટે રિસ્પોન્સિવ ટૅબ્સ (મોબાઈલમાં એક ઉપર એક આવશે)
        LayoutBuilder(
          builder: (context, constraints) {
            final isSmallScreen = constraints.maxWidth < 450;

            return Obx(() => isSmallScreen
                ? Column(
              children: [
                // 🇮🇳 પહેલી ટૅબ: ભારતમાં વસતા લોકો માટે
                SizedBox(
                  width: double.infinity,
                  child: InkWell(
                    onTap: () => controller.isIndiaTab.value = true,
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 8),
                      decoration: BoxDecoration(
                        color: controller.isIndiaTab.value ? AppColors.buttonPrimary : Colors.grey.shade200,
                        borderRadius: BorderRadius.circular(8),
                      ),
                      alignment: Alignment.center,
                      child: Text(
                        "🇮🇳 ભારતમાં વસતા લોકો માટે",
                        textAlign: TextAlign.center,
                        style: TextStyle(
                          color: controller.isIndiaTab.value ? Colors.white : Colors.black87,
                ...
                const SizedBox(height: 8),
                // 🌍 બીજી ટૅબ: ભારત બહાર વસતા લોકો માટે
                SizedBox(
                  width: double.infinity,
                  child: InkWell(
                    onTap: () => controller.isIndiaTab.value = false,
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 8),
                      decoration: BoxDecoration(
                        color: !controller.isIndiaTab.value ? AppColors.buttonPrimary : Colors.grey.shade200,
        ...
        // ==========================================
        // 📱 ટેબ ૧: ભારતમાં વસતા લોકો માટે (માત્ર મોબાઈલ + OTP)
        // ==========================================
        Obx(() => controller.isIndiaTab.value
            ? Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(child: _buildTextField(label: "OTP દાખલ કરો *", controller: controller.otpController)),
                const SizedBox(width: 12),
                ElevatedButton(
                  ...
                  onPressed: () async {
                    if (controller.phoneController.text.trim().length >= 10) {
                      ...
                      Get.snackbar("ભૂલ ❌", "સાચો મોબાઈલ નંબર નાખો!");
                    }
                  },
                  child: const Text("OTP મોકલો", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 30),
            ElevatedButton(
              ...
              onPressed: controller.isLoading.value ? null : () async {
                if (controller.otpController.text.trim().isNotEmpty) {
                  ...
                  Get.snackbar("ભૂલ ❌", "કૃપા કરીને OTP દાખલ કરો!");
                }
              },
              child: const Text("OTP વેરિફાઈ કરો ➔", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            ),
          ],
        )
        // ==========================================
        // 💻 ટેબ ૨: ભારત બહાર વસતા લોકો માટે (ઈમેલ + પાસવર્ડ)
        // ==========================================
            : Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: controller.isOldUser.value ? "તમારો લોગિન પાસવર્ડ દાખલ કરો *" : "પાસવર્ડ બનાવો *",
              ...
              onPressed: controller.isLoading.value ? null : () async {
                if (controller.isOldUser.value) {
                  ...
                  if (controller.passwordController.text.trim().isEmpty) {
                    ...
                    controller.nextStep(); // ➔ વેરિફિકેશન પૂરું થતાં જ આગળ જશે
                  }
                }
              },
              child: Text(
                controller.isOldUser.value ? "લોગિન કરો" : "આગળ વધો ➔",
  ...
  Widget _buildPersonalDetailsStep(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(child: _buildTextField(label: "WhatsApp નંબર", controller: controller.whatsappController)),
            const SizedBox(width: 16),
            Expanded(child: _buildTextField(label: "બ્લડ ગ્રુપ", controller: controller.bloodGroupController)),
          ],
        ),
        const SizedBox(height: 16),
  ...
  Widget _buildFamilyStep(BuildContext context) {
    ...
    bool shouldShowMaternal(String relation) {
      return ['પત્ની', 'માતા', 'પુત્રવધૂ', 'દાદી', 'બહેન', 'પુત્રી'].contains(relation);
    }
      ...
      if (memberCount == 0) {
        return Column(
          children: [
            ...
            const Icon(Icons.family_restroom, size: 70, color: Colors.grey),
            const SizedBox(height: 20),
            const Text("હજુ સુધી કોઈ સભ્ય ઉમેરેલ નથી", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 25),
            ElevatedButton.icon(
      ...
      return Column(
        ...
        children: [
          ...
          ...List.generate(memberCount, (index) {
            ...
            return Container(
              key: ValueKey('member_$reversedIndex'),
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildResponsiveRow([
                    ...
                    _buildTextField(label: "બ્લડ ગ્રુપ", controller: member['bloodGroupController'] ?? TextEditingController()),
                  ], context),
                  ...
                  // ➔ ⚡ શિક્ષણ, સ્માર્ટ વૈવાહિક સ્થિતિ અને વ્યવસાયના ડ્રોપડાઉન ફિલ્ડ્સ
                  _buildResponsiveRow([
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        const SizedBox(height: 6),
                        _buildCustomDropdown(
                          value: member['education'] ?? 'Select Option',
                          items: const ['Select Option', 'Under Graduate', 'Graduate', 'Post Graduate', 'Doctorate', 'Other'],
                          onChanged: (v) {
                            member['education'] = v!;
                            controller.familyMembers[reversedIndex] = Map<String, dynamic>.from(member);
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        isMarriedRelation(member['relation'])
                          ...
                          child: const Text("Married", style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold, fontSize: 14)),
                        )
                            : _buildCustomDropdown(
                          value: member['maritalStatus'] ?? 'Single',
                          items: const ['Single', 'Married', 'Divorced', 'Widowed'],
                          onChanged: (v) {
                            member['maritalStatus'] = v!;
                            controller.familyMembers[reversedIndex] = Map<String, dynamic>.from(member);
                            controller.familyMembers.refresh();
                          },
                        ),
                  ...
                  // ➔ ⚡ જો વ્યવસાય Business કે Job હોય તો પેઢી/કંપનીનું નામ
                  if (member['occupation'] == 'Business' || member['occupation'] == 'Job') ...[
                    const SizedBox(height: 16),
                    _buildTextField(
                      label: member['occupation'] == 'Business' ? "પેઢીનું નામ *" : "કંપનીનું નામ *",
                      controller: member['organizationNameController'] ?? TextEditingController(),
                    ),
                  ],

                  // ➔ ⚡ સ્ત્રી સભ્યો માટે પિયર પક્ષની વિગતો
                  if (shouldShowMaternal(member['relation'])) ...[
                    const SizedBox(height: 18),
                    const Divider(color: AppColors.divider, thickness: 1.5),
                    const SizedBox(height: 8),
                    const Row(
                      children: [
                        Icon(Icons.home_mini, color: AppColors.primary, size: 18),
                        SizedBox(width: 6),
                        Text(
                          "માતા ના પિયર પક્ષની વિગત",
                    ...
                    _buildResponsiveRow([
                      _buildTextField(label: "પિયરના પિતાનું પૂરું નામ *", controller: member['maternalFatherController']),
                      _buildTextField(label: "પિયરના માતાનું નામ *", controller: member['maternalMotherController']),
                    ], context),
                    const SizedBox(height: 12),
                    _buildTextField(label: "પિયરનું ગામ/શહેર *", controller: member['maternalVillageController']),
                  ],
                ],
              ),
            );
  ...
  Widget _buildVerificationStep() {
    return Column(
      ...
      children: [
        ...
        // મુખ્ય સભ્યની વિગતો પ્રિન્ટ કરો
        _buildReviewRow("નામ:", "${controller.firstNameController.text} ${controller.surnameController.text}"),
        _buildReviewRow("પિતા/પતિ:", controller.fatherHusbandController.text),
        _buildReviewRow("મોબાઈલ:", controller.phoneController.text),
        _buildReviewRow("ઈમેલ:", controller.emailController.text),
        _buildReviewRow("ગોત્ર:", controller.gotraController.text),
        _buildReviewRow("સરનામું:", controller.currentAddressController.text),
        _buildReviewRow("ગામ:", controller.currentCityVillageController.text),
        _buildReviewRow("જન્મ તારીખ:", "${controller.birthDayController.text}-${controller.birthMonthController.text}-${controller.birthYearController.text}"),
        _buildReviewRow("વૈવાહિક સ્થિતિ:", controller.selectedMaritalStatus.value),
        _buildReviewRow("શિક્ષણ:", controller.selectedEducation.value),
        _buildReviewRow("વ્યવસાય:", controller.selectedOccupation.value),

        const SizedBox(height: 20),
        const Text("પરિવારના સભ્યોની વિગતો:", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: AppColors.heading)),
        const SizedBox(height: 10),

        // પરિવારના સભ્યોની યાદી (ExpansionTile ડ્રોપડાઉન સાથે)
        controller.familyMembers.isEmpty
            ? const Text("કોઈ સભ્ય ઉમેરેલ નથી.", style: TextStyle(color: Colors.grey))
            : Column(
          children: controller.familyMembers.map((m) {
            return Card(
              ...
              child: ExpansionTile(
                title: Text(
                  m['nameController']?.text.isEmpty ?? true ? 'નામ વગરનો સભ્ય' : m['nameController']!.text,
                  style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppColors.heading),
                ),
                subtitle: Text("સંબંધ: ${m['relation'] ?? '-'}", style: const TextStyle(fontSize: 12, color: AppColors.subtitle)),
                children: [
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _buildReviewRow("જન્મ તારીખ:", "${m['dayController']?.text ?? ''}-${m['monthController']?.text ?? ''}-${m['yearController']?.text ?? ''}"),
                        _buildReviewRow("WhatsApp:", m['phoneController']?.text ?? '-'),
                        _buildReviewRow("બ્લડ ગ્રુપ:", m['bloodGroupController']?.text ?? '-'),
                        _buildReviewRow("શિક્ષણ:", m['education'] ?? '-'),
                        _buildReviewRow("વૈવાહિક સ્થિતિ:", m['maritalStatus'] ?? '-'),
                        _buildReviewRow("વ્યવસાય:", m['occupation'] ?? '-'),
                      ],
                    ),
                  ),
                ],
  ...
  // ==========================================
  // ➔ ⚡ ફિક્સ: Step 0 (વેરિફિકેશન પેનલ) પર ગ્લોબલ ફૂટર બટન છુપાઈ જશે!
  // ==========================================
  Widget _buildNavigationButtons() {
    return Obx(() {
      if (controller.isOldUser.value || controller.currentStep.value == 0) {
        return const SizedBox.shrink(); // Step 0 પર બટન ગાયબ!
      }
...
//
// // ==========================================
// // 📱 Step 1: Mobile & Email Verification (Final Fixed)
// // ==========================================
//   Widget _buildMobileStep(BuildContext context) {
//     // જો કંટ્રોલરમાં આ વેરીએબલ ન હોય તો તમારા Controller માં `var isIndiaTab = true.obs;` ઉમેરી દેજો.
//     // જો કંટ્રોલરનું નામ અલગ હોય તો તે મુજબ ચેક કરી લેજો.
//
//     return Column(
//       children: [
//         const Icon(Icons.security_rounded, size: 70, color: AppColors.iconAccent),
...
//             const SizedBox(height: 16),
//
//             // પાસવર્ડ ફિલ્ડ (લોગિન અથવા નવો પાસવર્ડ)
//             TextField(
//               controller: controller.passwordController,
//               obscureText: controller.isPasswordHidden.value,
//               style: const TextStyle(color: Colors.black87),
...
//             ),
//
//             // જો નવો યુઝર હોય તો કન્ફર્મ પાસવર્ડ બતાવો
//             if (!controller.isOldUser.value) ...[
//               const SizedBox(height: 16),
//               TextField(
//                 controller: controller.confirmPasswordController,
